...
public class Day4{
	public static void main(String[] args) {
		...
		char c='a';
		int newNumber,i=0,n=0,numberOfTables=0,winnerScore=0;
		try {
		...
		boolean [] loosingTables = new boolean[numberOfTables];
		for (i=0;i<numberOfTables;i++){
			loosingTables[i]=true;
		}
		for (newNumber=0;(newNumber<drawnNumbers.size());newNumber++){
			for (i=0;(i<numberOfTables);i++){
				if(loosingTables[i]){
					winnerScore=bT.get(i).addRevelation(drawnNumbers.get(newNumber));
					if((winnerScore>0)&&(loosingTables[i])){
						n=winnerScore;
						loosingTables[i]=false;
					}
				}
			}
		}
		System.out.println("score of last winning board "+n);
	}
}
class BingoTable{
